...
Public Class Polar
    ...
    Public Structure PolarCoordinates
        ...
        Public Overrides Function ToString() As String
            ...
            Dim s As String = ""
            Dim a As Double = _Angle, _
                r As Double = _Radius

            If Formatting And PolarCoordinateFormatting.UseParenthesis Then
                s &= "("
            End If

            If Formatting And PolarCoordinateFormatting.UseBrackets Then
                s &= "{"
            End If

            s &= Format(Math.Round(r, Rounding)) & ", "

            If Formatting And PolarCoordinateFormatting.DisplayInRadians Then
                a *= vbRadianConst
            End If

            s &= Format(Math.Round(a, Rounding))

            If Formatting And PolarCoordinateFormatting.UsePiSymbol Then
                s &= vbPiChar
            End If

            If Formatting And PolarCoordinateFormatting.UseDegreeSymbol Then
                s &= vbDegreeChar
            End If

            If Formatting And PolarCoordinateFormatting.UsePolarSymbol Then
                s &= vbPolarChar
            End If

            If Formatting And PolarCoordinateFormatting.UseRadianIndicator Then
                s &= " rad"
            End If
        ...
        Public Overloads Shared Function rad(p As PolarCoordinates) As System.Windows.Point
            Return rad(p, New Rectangle(0, 0, (p.Radius * 2) + 1, (p.Radius * 2) + 1))
        End Function

        Public Overloads Shared Function rad(p As PolarCoordinates, rect As Rectangle) As System.Windows.Point
            ...
            Dim r As Integer = p.Radius, _
                x As Integer, _
                y As Integer

            Dim c As Point = New Point(rect.Width / 2, rect.Height / 2)

            x = c.X + pt.X
            y = c.Y + pt.Y

            pt = New System.Windows.Point(x - 1, y - 1)
            Return New System.Windows.Point(x + rect.Location.X, y + rect.Location.Y)
