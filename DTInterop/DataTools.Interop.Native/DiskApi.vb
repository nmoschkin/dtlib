...
#Region "IOCTL_STORAGE"
        ...
        Public Const IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS As Integer = (((IOCTL_VOLUME_BASE) * (2 ^ 16)) Or ((FILE_ANY_ACCESS) * (2 ^ 14)) Or ((0) * (2 ^ 2)) Or (METHOD_BUFFERED))
...
#Region "Volume Management"
        ...
        Structure VOLUME_DISK_EXTENTS
            ...
            Public Shared Function FromPtr(ptr As IntPtr) As VOLUME_DISK_EXTENTS
                ...
                Dim cb As Integer = Marshal.SizeOf(New DISK_EXTENT)
                Dim m As MemPtr = ptr
                Dim i As Integer

                ve.NumberOfExtents = m.IntegerAt(0)
                ve.Space = m.IntegerAt(1)
                ReDim ve.Extents(ve.NumberOfExtents - 1)

                m += 8
                For i = 0 To ve.NumberOfExtents - 1
                    ve.Extents(i) = Marshal.PtrToStructure(m.Handle, GetType(DISK_EXTENT))
        ...
        Function GetDiskExtentsFor(devicePath As String) As DiskExtent()
            ...
            inBuff.SetLength(inSize)

            r = DeviceIoControl(file, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, 0, 0, inBuff, inSize, h, 0)
            If r = 0 AndAlso GetLastError = ERROR_MORE_DATA Then
                inBuff.SetLength(inSize * inBuff.IntegerAt(0))
                r = DeviceIoControl(file, IOCTL_VOLUME_GET_VOLUME_DISK_EXTENTS, 0, 0, inBuff, inSize, h, 0)
            End If
            ...
            For Each de In ve.Extents
                ReDim Preserve deOut(r)
                deOut(r).PhysicalDevice = de.DiskNumber
                deOut(r).Space = de.Space
                deOut(r).Size = de.ExtentLength
                deOut(r).Offset = de.StartingOffset
                r += 1
            Next
        ...
        Public Sub PopulateVolumeInfo(ByRef disk As DiskDeviceInfo, Optional handle As IntPtr = Nothing)
            ...
            disk.VolumeGuidPath = mm1
            disk.VolumePaths = GetVolumePaths(mm1)
        ...
        Public Function GetVolumePaths(path As String) As String()
            ...
            GetVolumePaths = mm.GrabStringArray(0)
            mm.Free()
...
#End Region
    ...
    Module RawDisk
        ...
        Public Function ReadRawGptDisk(inf As DiskDeviceInfo, ByRef gptInfo As RAW_GPT_DISK) As Boolean
            ...
            Dim bps As Integer = geo.Geometry.BytesPerSector
            ...
            ReadFile(hfile, mm.Handle, bps * 2, br, IntPtr.Zero)
            ...
            If gpt.IsValid Then
                ...
                SetFilePointerEx(hfile, bps * gpt.PartitionEntryLBA, br, FilePointerMoveMethod.Begin)
                ...
                ReadFile(hfile, mm.Handle, lp, br, IntPtr.Zero)

                '' check the partition table CRC.
                If mm.CalculateCrc32 = gpt.PartitionArrayCRC32 Then
                    '' disk is valid.

                    lp = Marshal.SizeOf(New RAW_GPT_PARTITION)
                    br = 0

                    Dim i As Integer
                    Dim c As Integer = gpt.NumberOfPartitions - 1
        ...
        Public Structure RAW_MBR
            ...
            Public ReadOnly Property StartingCHS As Integer
                Get
                    ...
                    MemCpy(mm.Handle, _startingChs, 3)
                    StartingCHS = mm.IntegerAt(0)
                    mm.Free()
                End Get
            End Property
            ...
            Public ReadOnly Property EndingCHS As Integer
                Get
                    Dim mm As New MemPtr(4)
                    MemCpy(mm.Handle, _endingChs, 3)
                    EndingCHS = mm.IntegerAt(0)
                    mm.Free()
                End Get
            End Property
    ...
    Module Partitioning
        ...
        Public Structure PARTITION_INFORMATION_EX
            ...
            Public ReadOnly Property Mbr As PARTITION_INFORMATION_MBR
                Get
                    ...
                    Mbr = Nothing
                    BytesToStruct(_PartitionInfo, Mbr)
                End Get
            End Property
            ...
            Public ReadOnly Property Gpt As PARTITION_INFORMATION_GPT
                Get
                    ...
                    Gpt = Nothing
                    BytesToStruct(_PartitionInfo, Gpt)
                End Get
            End Property
            ...
            Public Overrides Function ToString() As String
                ...
                If PartitionStyle = PartitionStyle.Mbr Then
                    ToString = Mbr.ToString & " [" & CType(PartitionLength, FriendlySizeLong).ToString & "]"
                Else
                    ToString = Gpt.ToString & " [" & CType(PartitionLength, FriendlySizeLong).ToString & "]"
                End If
            End Function
        ...
        Public Function GetPartitions(devicePath As String, Optional hfile As IntPtr = Nothing, Optional ByRef layInfo As DRIVE_LAYOUT_INFORMATION_EX = Nothing) As PARTITION_INFORMATION_EX()
            ...
            Dim pexLen As Integer = Marshal.SizeOf(New PARTITION_INFORMATION_EX)
            ...
            lay = Marshal.PtrToStructure(pex.Handle, GetType(DRIVE_LAYOUT_INFORMATION_EX))

            pexBegin.Handle = pex.Handle + 48

            c = lay.ParititionCount - 1
            ReDim pOut(c)

            For i = 0 To c
                pOut(i) = Marshal.PtrToStructure(pexBegin.Handle, GetType(PARTITION_INFORMATION_EX))
                pexBegin += pexLen
            Next
