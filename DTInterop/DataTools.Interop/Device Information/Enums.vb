...
Public Module DevClassPresenting
    ...
    Friend Function GetDevInterfaceClassEnumFromGuid(devInterface As Guid) As DeviceInterfaceClassEnum
        Dim i As DeviceInterfaceClassEnum = 0

        If devInterface = GUID_DEVINTERFACE_BRIGHTNESS Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_DISPLAY_ADAPTER Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_I2C Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_IMAGE Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_MONITOR Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_OPM Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_HID Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_KEYBOARD Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_MOUSE Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_MODEM Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_NET Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_SENSOR Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_COMPORT Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_PARALLEL Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_PARCLASS Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_SERENUM_BUS_ENUMERATOR Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_CDCHANGER Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_CDROM Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_DISK Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_FLOPPY Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_MEDIUMCHANGER Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_PARTITION Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_STORAGEPORT Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_TAPE Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_VOLUME Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_WRITEONCEDISK Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_USB_DEVICE Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_USB_HOST_CONTROLLER Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_USB_HUB Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_WPD Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_WPD_PRIVATE Then Return i
        i += 1

        If devInterface = GUID_DEVINTERFACE_SIDESHOW Then Return i
    ...
    Friend Function GetDevClassEnumFromGuid(devClass As Guid) As DeviceClassEnum
        ...
        '' classes
        If devClass = GUID_DEVCLASS_1394 Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_1394DEBUG Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_61883 Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_ADAPTER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_APMSUPPORT Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_AVC Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_BATTERY Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_BIOMETRIC Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_BLUETOOTH Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_CDROM Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_COMPUTER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_DECODER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_DISKDRIVE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_DISPLAY Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_DOT4 Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_DOT4PRINT Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_ENUM1394 Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_FDC Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_FLOPPYDISK Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_GPS Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_HDC Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_HIDCLASS Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_IMAGE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_INFINIBAND Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_INFRARED Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_KEYBOARD Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_LEGACYDRIVER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MEDIA Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MEDIUM_CHANGER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MEMORY Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MODEM Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MONITOR Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MOUSE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MTD Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MULTIFUNCTION Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_MULTIPORTSERIAL Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_NET Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_NETCLIENT Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_NETSERVICE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_NETTRANS Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_NODRIVER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PCMCIA Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PNPPRINTERS Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PORTS Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PRINTER_QUEUE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PRINTER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PRINTERUPGRADE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_PROCESSOR Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SBP2 Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SCSIADAPTER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SECURITYACCELERATOR Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SENSOR Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SIDESHOW Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SMARTCARDREADER Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SOUND Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_SYSTEM Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_TAPEDRIVE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_UNKNOWN Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_USB Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_VOLUME Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_VOLUMESNAPSHOT Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_WCEUSBS Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_WPD Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_EHSTORAGESILO Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_FIRMWARE Then Return i
        i += 1

        If devClass = GUID_DEVCLASS_EXTENSION Then Return i
        i += 1
    ...
    Public Structure DiskExtent
        ...
        Public Overrides Function ToString() As String
            Return "Physical Device " & PhysicalDevice & ", " & PrintFriendlySize(Size)
        End Function
