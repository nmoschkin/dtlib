...
#End Region
    ...
    Public NotInheritable Class PartitionCodeInfo
        ...
        Public Overrides Function ToString() As String
            ...
            If Name Is Nothing Then
                If _OS Is Nothing OrElse _OS.Count = 0 Then
                    ...
                    End If
                Else
                    Dim sb As DataTools.Memory.StringBlob = _OS
                    ToString = sb.ToFormattedString(DataTools.Memory.StringBlobFormats.Spaced Or DataTools.Memory.StringBlobFormats.Commas)
                    sb.Dispose()
                End If
            Else
                Return Name
        ...
        Private Sub Parse(s As String)
            ...
            Dim vs() As String = BatchParse(s, ",", True, True, """", """", False, False, True)
            ...
            _Occurrence = FlagsParse(Of PartitionOccurrence)(vs(1))
            _Access = FlagsParse(Of PartitionAccess)(vs(2))
            _Bootability = _internalParseBootFlags(vs(3))
            _Characteristics = FlagsParse(Of PartitionCharacteristics)(vs(4))
            _Origins = BatchParse(vs(5), ", ")
        ...
        Private Shared Function _internalParseBootFlags(s As String) As PartitionBootability
            ...
            For Each x In vs
                ...
                If sl.Contains(x) Then
                    vl = vl Or [Enum].Parse(vl.GetType, x)
                ElseIf sl.Contains("M" & x) Then
                    vl = vl Or [Enum].Parse(vl.GetType, "M" & x)

                ElseIf sl.Contains("I" & x) Then
                    vl = vl Or [Enum].Parse(vl.GetType, "I" & x)
        ...
        Public Function Compare(x As PartitionCodeInfo, y As PartitionCodeInfo) As Integer Implements IComparer(Of PartitionCodeInfo).Compare
            ...
            If x.PartitionID.Value = y.PartitionID.Value Then
                ...
                ElseIf los1.Contains("Windows NT") AndAlso los2.Contains("Windows NT") Then
                    Return String.Compare(x.Description, y.Description)
                ElseIf los1.Contains("Windows NT") >= 0 Then
                    Return -1
                ElseIf los2.Contains("Windows NT") >= 0 Then
                    Return 1
                Else
                    Return String.Compare(x.Description, y.Description)
                End If
...
#End Region
    ...
    Public MustInherit Class DiskPartitionInfo
        ...
        Public ReadOnly Property PartitionNumber As Integer Implements IDiskPartition.PartitionNumber
            Get
                Return _partex.PartitionNumber
            End Get
        End Property
