...
#Region "DiskDeviceInfo"
    ...
    Public Class DiskDeviceInfo
        ...
        Public ReadOnly Property IsVolumeMounted As Boolean
            Get
                Return GetDriveFlag() And GetLogicalDrives <> 0
            End Get
        End Property
        ...
        Private Function GetDriveFlag() As UInteger
            If Not IsVolume Then Return False

            If _VolumePaths Is Nothing OrElse _VolumePaths.Length = 0 Then Return False
        ...
        Public Overrides Function ToString() As String
            If IsVolume Then
                If VolumePaths IsNot Nothing AndAlso VolumePaths.Count > 0 Then
                    Dim sb As StringBlob = VolumePaths
                    ToString = "[" & sb.ToFormattedString(StringBlobFormats.Commas) & "] "
                Else
                    ToString = ""
                End If
                ToString &= FriendlyName & " (" & PrintFriendlySize(Size) & ")"
            Else
                ToString = "[" & Type.ToString & "] " & FriendlyName & " (" & PrintFriendlySize(Size) & ")"
            End If
