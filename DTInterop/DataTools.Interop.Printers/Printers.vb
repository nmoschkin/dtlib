...
#Region "Declares"
        ...
        Function OpenPrinter(<MarshalAs(UnmanagedType.LPWStr)> ByVal pPrinterName As String, ByRef hPrinter As IntPtr, ByVal pDefault As IntPtr) As Boolean
        End Function
        ...
        Function EnumPrinterKey(hPrinter As IntPtr, _
                                <MarshalAs(UnmanagedType.LPWStr)> pKeyName As String, _
                                pSubkey As IntPtr,
...
#Region "Convoluted Printer Capabilities Deducing Functions"
        ...
        Public Function GetMaxDPIForPrinter(printer As PrinterObject, Optional hPrinter As IntPtr = Nothing) As System.Windows.Size
...
#End Region
        ...
        Private Shared Sub ParsePapers()
            ...
            Dim paperList() As String = BatchParse(_SizeDataString, vbCrLf)

            For Each paper In paperList
                Dim p As New SystemPaperType

                Dim data() As String = BatchParse(paper, vbTab)

                p.Name = CamelCase(data(0))
                p.Code = CInt(data(1))
        ...
        Private Shared Function FindSize(text As String, ByRef isMM As Boolean, Optional scanforDblQuote As Boolean = False, Optional acceptComma As Boolean = False) As System.Windows.Size
            ...
            For Each num In sizes
                d = 0
                Dim nch() As String = BatchParse(num.Trim, " ")

                If nch.Count = 2 Then
                    Dim div() As String = BatchParse(nch(1), "/")
...
#Region "JobInfo Class"
    ...
    Public Class JobInfo
        ...
        Public Overrides ReadOnly Property IsInvalid As Boolean
            Get
                Return (_ptr.Handle = 0)
            End Get
        End Property

        Protected Overrides Function ReleaseHandle() As Boolean
            Try
                If _ptr.Handle <> 0 Then _ptr.Free()
                Return True
            Catch ex As Exception
                Return False
            End Try
        ...
        Public Property StatusMessage As String
            ...
            End Get
            Set(value As String)
                _str.StringIndirectAt(10) = value
            End Set
        End Property
        ...
        Public Property SecurityDescriptor As IntPtr
            ...
            Set(value As IntPtr)
                If IntPtr.Size = 4 Then
                    _str.IntegerAt(11) = value
                Else
                    _str.LongAt(11) = value
                End If
            End Set
        End Property
...
#Region "Printer Object Collection"
    ...
    Public Class PrinterObjects
        ...
        Public Shared Function Refresh() As Boolean
            ...
            EnumPrinters(PRINTER_ENUM_NAME, "", 4, 0, 0, cb, rc)
            ...
            If cb > 0 Then
                ...
                For u As Integer = 1 To rc
                    ts = ap.GrabStringFromPointerAtAbsolute(cb)
                    sp.Add(ts)
                    ap += ps
                Next
...
#Region "Object Stuff (New, Finalize, Dispose, Equals, ToString, etc...)"
        ...
        Friend Sub New(ptr As IntPtr, fOwn As Boolean)
            ...
            If IntPtr.Size = 4 Then
                _DevMode = New DeviceMode(CType(mm.IntegerAt(7), IntPtr), False)
...
#Region "Shared Methods"
        ...
        Public Shared Function GetPrintableArea(printer As PrinterObject, paper As SystemPaperType, resolution As UniSize, Optional orientation As Integer = 0) As UniRect
            ...
            Dim dev As DeviceMode = printer._DevMode.Clone

            dev.Fields = DeviceModeFields.Orientation Or DeviceModeFields.PaperSize Or DeviceModeFields.YResolution

            dev.YResolution = resolution.cy
            dev.PrintQuality = resolution.cx

            dev.PaperSize = paper
            dev.Orientation = orientation

            Dim hdc = CreateDC(Nothing, printer.PrinterName, 0, printer._DevMode._ptr)
        ...
        Private Shared Sub internalGetPrinter(name As String, ByRef printer As PrinterObject)
            ...
            If Not OpenPrinter(name, hprinter, 0) Then

                MsgBox("OpenPrinter failed, last Win32 Error: " & FormatLastError(System.Runtime.InteropServices.Marshal.GetLastWin32Error))
                Return
            ...
            Try
                ...
                If IntPtr.Size = 4 Then
                    printer._DevMode = New DeviceMode(CType(mm.IntegerAt(7), IntPtr), False)
                    '   MsgBox("Got a pointer! x86 mode.")
                Else
        ...
        Private Shared Sub internalPopulateDevCaps(ByRef printer As PrinterObject)
            Try
                ...
                Dim res() As UInteger
                Dim l As UInteger = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_ENUMRESOLUTIONS, 0, 0)
                ...
                If printer.PrinterName.Contains("HP LaserJet") Then
                    ...
                    If l = &HFFFFFFFFUI Then
                        ...
                        l = DeviceCapabilities(printer.PrinterName, Nothing, DC_ENUMRESOLUTIONS, 0, 0)
                    ...
                    If (l > 0) And (l <> &HFFFFFFFFUI) Then
                        ...
                        l = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_ENUMRESOLUTIONS, mm, 0)
                ...
                l = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_PAPERS, 0, 0)

                '' supported paper types are short ints:
                If (l > 0) Then
                    mm = New MemPtr(l * 2)

                    l = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_PAPERS, mm, 0)
                    printer._PaperSizes = SystemPaperTypes.TypeListFromCodes(CType(mm, Short()))
                ...
                l = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_BINNAMES, 0, 0)

                If l > 0 Then
                    mm = New MemPtr(l * 24 * 2)
                    mm.ZeroMemory()

                    DeviceCapabilities(printer.PrinterName, printer.PortName, DC_BINNAMES, mm, 0)
                    printer._Bins.Clear()
                ...
                l = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_COLORDEVICE, 0, 0)
                printer.IsColorPrinter = CBool(l)

                printer._LandscapeRotation = DeviceCapabilities(printer.PrinterName, printer.PortName, DC_ORIENTATION, 0, 0)
...
#Region "DeviceCapabilities Properties"
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Property Resolutions As IEnumerable(Of System.Windows.Size)
            Get
                Return _Resolutions
            End Get
            Friend Set(value As IEnumerable(Of System.Windows.Size))
                _Resolutions = value
            End Set
        End Property
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Property PaperSizes As IEnumerable(Of SystemPaperType)
            Get
                Return _PaperSizes
            End Get
            Friend Set(value As IEnumerable(Of SystemPaperType))
                _PaperSizes = value
            End Set
        End Property
        ...
        ''' <remarks></remarks>
        Public Function SupportsPaperSize(size As System.Windows.Size, Optional ByVal sizeMetric As Boolean = False, Optional ByVal exactOrientation As Boolean = False) As Boolean
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Public Property PrinterTrays As IEnumerable(Of String)
            Get
                Return _Bins
            End Get
            Friend Set(value As IEnumerable(Of String))
                _Bins = value
            End Set
        End Property
...
#Region "Properties based on PRINTER_INFO_2"
        ...
        Public Property ServerName As String
            Get
                Return _ptr.StringIndirectAt(0)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(0) = value
            End Set
        End Property
        ...
        Public Property PrinterName As String
            Get
                Return _ptr.StringIndirectAt(1)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(1) = value
            End Set
        End Property
        ...
        Public Property ShareName As String
            Get
                Return _ptr.StringIndirectAt(2)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(2) = value
            End Set
        End Property
        ...
        Public Property PortName As String
            Get
                Return _ptr.StringIndirectAt(3)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(3) = value
            End Set
        End Property
        ...
        Public Property DriverName As String
            Get
                Return _ptr.StringIndirectAt(4)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(4) = value
            End Set
        End Property
        ...
        Public Property Comment As String
            Get
                Return _ptr.StringIndirectAt(5)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(5) = value
            End Set
        End Property
        ...
        Public Property Location As String
            Get
                Return _ptr.StringIndirectAt(6)
            End Get
            Friend Set(value As String)
                _ptr.StringIndirectAt(6) = value
            End Set
        End Property
        ...
        Public Property SepFile As String
            Get
                Return _ptr.StringIndirectAt(8)
            End Get
            Set(value As String)
                _ptr.StringIndirectAt(8) = value
            End Set
        End Property
        ...
        Public Property PrintProcessor As String
            Get
                Return _ptr.StringIndirectAt(9)
            End Get
            Set(value As String)
                _ptr.StringIndirectAt(9) = value
            End Set
        End Property
        ...
        Public Property Datatype As String
            Get
                Return _ptr.StringIndirectAt(10)
            End Get
            Set(value As String)
                _ptr.StringIndirectAt(10) = value
            End Set
        End Property
        ...
        Public Property Parameters As String
            Get
                Return _ptr.StringIndirectAt(11)
            End Get
            Set(value As String)
                _ptr.StringIndirectAt(11) = value
            End Set
        End Property
        ...
        Public Property SecurityDescriptor As IntPtr
            Get
                Return _ptr.LongAt(12)
            End Get
            Friend Set(value As IntPtr)
                _ptr.LongAt(12) = value
            End Set
        End Property
        ...
        Public Property Attributes As PrinterAttributes
            Get
                Return _ptr.UIntegerAt(26)
            End Get
            Set(value As PrinterAttributes)
                _ptr.UIntegerAt(26) = value
            End Set
        End Property
        ...
        Public Property Priority As UInteger
            Get
                Return _ptr.UIntegerAt(27)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAt(27) = value
            End Set
        End Property
        ...
        Public Property DefaultPriority As UInteger
            Get
                Return _ptr.UIntegerAt(28)
            End Get
            Friend Set(value As UInteger)
                _ptr.UIntegerAt(28) = value
            End Set
        End Property
        ...
        Public Property StartTime As TimeSpan
            Get
                Return New TimeSpan(0, _ptr.UIntegerAt(29), 0)
            End Get
            Friend Set(value As TimeSpan)
                _ptr.UIntegerAt(29) = value.TotalMinutes
            End Set
        End Property
        ...
        Public Property UntilTime As TimeSpan
            Get
                Return New TimeSpan(0, _ptr.UIntegerAt(30), 0)
            End Get
            Friend Set(value As TimeSpan)
                _ptr.UIntegerAt(30) = value.TotalMinutes
            End Set
        End Property
        ...
        Public Property Status As PrinterStatus
            Get
                ...
                Return _ptr.UIntegerAt(31)
            End Get
            Friend Set(value As PrinterStatus)
                _ptr.UIntegerAt(31) = value
            End Set
        End Property
        ...
        Public Function UpdateStatus() As PrinterStatus
            ...
            If Not OpenPrinter(PrinterName, hprinter, 0) Then Return PrinterStatus.Error

            GetPrinter(hprinter, 6, 0, 0, cb)
            ...
            _ptr.UIntegerAt(31) = _mm.UIntegerAt(0)
        ...
        Public Property cJobs As UInteger
            Get
                Return _ptr.UIntegerAt(32)
            End Get
            Friend Set(value As UInteger)
                _ptr.UIntegerAt(32) = value
            End Set
        End Property
        ...
        Public Property AveragePPM As UInteger
            Get
                Return _ptr.UIntegerAt(33)
            End Get
            Friend Set(value As UInteger)
                _ptr.UIntegerAt(33) = value
            End Set
        End Property
    ...
    Public Class DeviceMode
        ...
        Public Property Fields As DeviceModeFields
            ...
            Set(value As DeviceModeFields)
                _ptr.UIntegerAtAbsolute(72) = value
            End Set
        End Property
        ...
        Public Property Position As System.Drawing.Point
            Get
                Return New System.Drawing.Point(_ptr.IntegerAtAbsolute(76), _ptr.IntegerAtAbsolute(80))
            End Get
            Set(value As System.Drawing.Point)
                _ptr.IntegerAtAbsolute(76) = value.X
                _ptr.IntegerAtAbsolute(80) = value.Y
            End Set
        End Property

        Public Property DisplayOrientation As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(84)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(84) = value
            End Set
        End Property

        Public Property DisplayFixedOutput As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(88)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(88) = value
            End Set
        End Property
        ...
        Public Property FormName As String
            Get
                Return _ptr.GrabString(102, 32).Trim(ChrW(0))
            End Get
            Set(value As String)
                If (value.Length > 32) Then value = value.Substring(0, 32)
        ...
        Public Property BitsPerPel As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(170)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(170) = value
            End Set
        End Property

        Public Property PelsWidth As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(174)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(174) = value
            End Set
        End Property

        Public Property PelsHeight As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(178)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(178) = value
            End Set
        End Property
        ...
        Public Property DisplayFlags As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(182)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(182) = value
            End Set
        End Property
        ...
        Public Property DisplayFrequency As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(186)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(186) = value
            End Set
        End Property

        Public Property ICMMethod As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(190)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(190) = value
            End Set
        End Property

        Public Property ICMIntent As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(194)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(194) = value
            End Set
        End Property

        Public Property MediaType As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(198)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(198) = value
            End Set
        End Property

        Public Property DitherType As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(202)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(202) = value
            End Set
        End Property

        Public Property Reserved1 As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(206)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(206) = value
            End Set
        End Property

        Public Property Reserved2 As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(210)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(210) = value
            End Set
        End Property

        Public Property PanningWidth As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(214)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(214) = value
            End Set
        End Property

        Public Property PanningHeight As UInteger
            Get
                Return _ptr.UIntegerAtAbsolute(218)
            End Get
            Set(value As UInteger)
                _ptr.UIntegerAtAbsolute(218) = value
            End Set
        End Property
