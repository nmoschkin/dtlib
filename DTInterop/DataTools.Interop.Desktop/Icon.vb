...
Namespace Desktop
    ...
    Public Class IconImageEntry
        ...
        Friend Sub New(ptr As IntPtr)
            ...
            _entry = Marshal.PtrToStructure(ptr, GetType(ICONDIRENTRY))
            ptr += _entry.dwOffset
        ...
        Public Function ToIcon() As Icon
            If IsPngFormat Then
                ...
                Dim z As Integer = ((Math.Max(bmp.Width, 32) * bmp.Height) / 8)
                ...
                If hIcon <> 0 Then
                    ToIcon = Icon.FromHandle(hIcon).Clone
                    DestroyIcon(hIcon)
                Else
                    ToIcon = Nothing
                End If
        ...
        Public ReadOnly Property IsPngFormat As Boolean
            Get
                ...
                Dim mm As SafePtr = _image

                Dim q As Integer = mm.IntegerAt(0)
        ...
        Private Function _makeBitmap() As Byte()
            ...
            Dim hbmp As IntPtr = MakeDIBSection(ToImage, bmp)
            ...
            maskSize = CLng((Math.Max(w, 32) * h) / 8)
            mm.Length = bm.biSizeImage + 40 + maskSize
            ...
            MemCpy(ptr1, bmp, bm.biSizeImage)

            bm = Marshal.PtrToStructure(mm.DangerousGetHandle, GetType(BITMAPINFOHEADER))
            _setMask(ptr1, ptr2, w, h)

            _entry.dwImageSize = mm.Length
        ...
        Private Function _constructIcon() As Icon
            ...
            Dim mm As MemPtr = _image
            Dim bmp As BITMAPINFOHEADER = Marshal.PtrToStructure(mm.Handle, GetType(BITMAPINFOHEADER))
            Dim hBmp As IntPtr
            Dim ptr As IntPtr
            ...
            bmp.biSizeImage = bmp.biWidth * bmp.biHeight * (bmp.biBitCount / 8)
            bmp.biXPelsPerMeter = 24.5 * 1000
            bmp.biYPelsPerMeter = 24.5 * 1000
            bmp.biClrUsed = 0
            ...
            hBmp = CreateDIBSection(0, mm.Handle, 0, ppBits, 0, 0)
            MemCpy(ppBits, ptr, bmp.biSizeImage)
            ...
            If hasMask Then
                ...
                bmp.biSizeImage = 0
                Marshal.StructureToPtr(bmp, mm.Handle, False)
                hBmpMask = CreateDIBSection(0, mm.Handle, 0, ppBits, 0, 0)

                MemCpy(ppBits, ptr, CLng((Math.Max(bmp.biWidth, 32) * bmp.biHeight) / 8))
            End If
        ...
        Private Sub _setMask(hBits As MemPtr, hMask As MemPtr, Width As Integer, Height As Integer)
            ...
            Dim stride As Integer = Width * (numBits / 8)
            Dim msize As Integer = CInt((Math.Max(32, Width) * Height) / 8)
            Dim isize As Integer = ((Width * Height) * (numBits / 8))
            ...
            MemCpy(imgb, hBits.Handle, isize)
        ...
        ''' <remarks></remarks>
        Public Sub New(Image As Image, size As StandardIcons, Optional ByVal asBmp As Boolean = False)

            Dim sz As Integer = If(size, CType(size, Integer) And &HFF, 256)
            Dim im As Bitmap = Image
...
#End Region
    ...
    Public Class IconImage
        ...
        ''' <returns>True if successful.</returns>
        ''' <remarks></remarks>
        Public Function SaveIcon(ByVal fileName As String) As Boolean
        ...
        ''' <returns>True if successful.</returns>
        ''' <remarks></remarks>
        Public Function SaveIcon(ByVal stream As Stream) As Boolean
        ...
        Private Function _internalLoadFromPtr(ptr As IntPtr) As Boolean
            ...
            _dir = Marshal.PtrToStructure(ptr, GetType(ICONDIR))

            Dim i As Integer,
            c As Integer = _dir.nImages - 1

            Dim f As Integer = Marshal.SizeOf(New ICONDIRENTRY)
            Dim e As Integer = Marshal.SizeOf(New ICONDIR)
            ...
            For i = 0 To c
                ...
                _entries.Add(New IconImageEntry(Marshal.PtrToStructure(ptr, GetType(ICONDIRENTRY)), optr))
                ptr += f
            Next
        ...
        Private Function _internalSaveToStream(stream As Stream) As Boolean
            ...
            Dim g As Integer = Marshal.SizeOf(New ICONDIRENTRY)

            _dir.nImages = _entries.Count
            ...
            '' write the icon file
            stream.Write(CType(bl, Byte()), 0, bl.Length)
            stream.Close()
            bl.Dispose()
